...
var cobolscript = (function() {
    ...
    return {
        ...
        run: function(text, runtime, program) {        
            ...
            if (text)
                procedure = compileFunction(text);
            else
                procedure = program.procedure;
            
            return procedure(runtime, program);
        },        compileProgram: compileProgram,
        compileTemplate: compileTemplate,
        compileProgramFile: function(filename) {
            var text = fs.readFileSync(filename).toString();
            return compileProgram(text);
        },
        compileTemplateFile: function(filename) {
            var text = fs.readFileSync(filename).toString();
            return compileTemplate(text);
        }
    };
})();
