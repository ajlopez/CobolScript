...
var cobolscript = (function() {
    ...
    function ProcedureCommand(name, commands)
    {
        this.compile = function(program) {
            var code = 'function ' + name + '() {';
            if (commands)
                code += commands.compile(program);
            code += '};';
            return code;
        };
    };
    ...
    function PerformCommand(name, options)
    {
        ...
        this.compile = function(program) {
            ...
            if (options.varying) {
                var variable = options.varying.compile(program);
                var step = options.by.compile(program);
                var oper = '<=';
                var incr = '+= ' + step;
                ...
                code = 'for (' + variable + ' = ' + options.from.compile(program) + '; ' + variable + ' ' + oper + ' ' + options.to.compile(program) + '; ' + variable + incr + ') ' + code;
            }
            
            return code;
        };
    }        function CompositeCommand()    {        var commands = [];                this.add = function(cmd) {            commands.push(cmd);        };                this.compile = function(program) {            var code = '';            var ncmd = commands.length;            for (var k = 0; k < ncmd; k++)                code += commands[k].compile(program) + '\r\n';                            return code;        };    };        function DisplayCommand(expr, noadvancing)
    {
        this.compile = function(program) {
            if (noadvancing)
                return 'runtime.write(' + compileExpression(program) + ');';
            else
                return 'runtime.display(' + compileExpression(program) + ');';
        }

        function compileExpression(program) {
            if (!(expr instanceof Array))
                return expr.compile(program);
        ...
        this.compile = function(program) {
            if (!(variable instanceof Array))
                return compileTarget(program, variable);
                
            var n = variable.length;
            code = '';
            
            for (var k = 0; k < n; k++)
                code += compileTarget(program, variable[k]);
            return code;        };
        
        function compileTarget(program, target) {
            var code = target.compile(program) + ' = ';

            if (!giving)
                code += target.compile(program) + ' ' + oper + ' ';
            else if (from)
                code += from.compile(program) + ' ' + oper + ' ';

            code += compileExpression(program) + ';';
