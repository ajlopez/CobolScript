...
var cobolscript = (function () {
    ...
    function IndexedVariableExpression(name, index)
    {
        ...
        this.compile = function(program, context) {
            var code = variable.compile(program, context);
            code += '[';
    ...
    function ProcedureCommand(name, commands, args, options)
    {
        ...
        function getContext(context) {
            ...
            var context = { arguments: { }, locals: { }, options: opts, async: options.async };
    ...
    function InlinePerformCommand(commands, options)
    {
        ...
        this.compile = function(program, context) {
            ...
            if (options.varying) {
                ...
                code = 'for (' + forvariable + ' = ' + options.from.compile(program, context) + '; ' + forvariable + ' ' + oper + ' ' + options.to.compile(program) + '; ' + forvariable + ' ' + incr + ')';
            }
            else {
                code = 'while (';
    ...
    function PerformCommand(variable, options)
    {
        ...
        this.compile = function(program, context) {
            ...
            if (options.varying) {
                ...
                code = 'for (' + forvariable + ' = ' + options.from.compile(program, context) + '; ' + forvariable + ' ' + oper + ' ' + options.to.compile(program) + '; ' + forvariable + incr + ') ' + code;
            }
    ...
    function Parser(text) {
        ...
        this.parseProcedure = function(procedures) {
            ...
            // If not a section, their commands are the continuation of the preceding ones
            if (!tryParseName('section')) {
                parsePoint();
                return this.parseCommands();
            }
