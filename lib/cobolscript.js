...
var cobolscript = (function() {
    ...
    function ProcedureCommand(name, commands, args, locals)
    {
        ...
        function getContext() {
            ...
            if (locals) {
                ...
                for (var k = 0; k < n; k++)
                    context.locals[locals[k].getName()] = null;
            }
    ...
    function Parser(text) {
        ...
        this.parseProcedure = function() {
            ...
            if (tryParseName("locals") || tryParseName("local"))
                locals = parseVariables();
            
            parsePoint();
            
            var cmds = this.parseCommands();
            
            return new ProcedureCommand(token.value, cmds, arguments, locals);
        ...
        function parseProcedureDivision() {
            ...
            var command = self.parseProcedures();
    ...
    function compileTemplateCode(text) {
        var begin = text.indexOf("<#");
        ...
        if (begin >= 0 && end > begin) {
            ...
            if (left)
                result += 'display ' + compileTemplateExpression(left);
            
            result += text.slice(begin + 2, end);
            
            if (right)
                result += compileTemplateCode(right);
                
            return result;
        }
        
        return 'display ' + compileTemplateExpression(text);
    };
    ...
    return {
        ...
        compileTemplate: function(text) {
            return compileTemplateCode(text);
        }
    };
})();
